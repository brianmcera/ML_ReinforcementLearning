...
class sample_gaussian_input(layers.Layer):
    ...
    def build(self,input_shape):
        self.std = 1e-2*np.identity(self.ac_dim) # identity for now, make trainable later
...
def main():
    ...
    for _ in range(training_epochs):
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices((obs_data[:-num_samples//split_size],
                acs_data[:-num_samples//split_size],10*reward_to_go[:-num_samples//split_size])).shuffle(10000).batch(32)
