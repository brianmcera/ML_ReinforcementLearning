...
class policyMu(Model):
    def __init__(self, ob_dim, ac_dim):
        ...
        self.d1 = Dense(3, input_shape=ob_dim, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(0.00))
        self.d2 = Dense(3, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(0.00))
        self.d3 = Dense(3, activation='tanh', kernel_regularizer=tf.keras.regularizers.l2(0.00))
        self.d4 = Dense(ac_dim, kernel_regularizer=tf.keras.regularizers.l2(0.00))
...
class baselineNN(Model):
    def __init__(self, ob_dim):
        ...
        self.d1 = Dense(20, input_shape=ob_dim, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.d2 = Dense(10, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.d3 = Dense(10, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))
        self.dropout = Dropout(rate=0.5)
        self.d4 = Dense(1) 
...
def main():
    ...
    #optimizer1 = tf.keras.optimizers.Adam(epsilon=1)
    #optimizer2 = tf.keras.optimizers.Adam(epsilon=1)
    optimizer1 = tf.keras.optimizers.SGD(learning_rate=1e-4, momentum=0.8)
    optimizer2 = tf.keras.optimizers.SGD(learning_rate=1e-4, momentum=0.8)
    ...
    for _ in range(training_epochs):
        ...
        avg_return_vec = np.append(avg_return_vec, np.mean(data['returns']))
        std_return_vec = np.append(std_return_vec, np.std(data['returns']))
