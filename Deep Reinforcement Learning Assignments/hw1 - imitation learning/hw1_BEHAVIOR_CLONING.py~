    ...
    with tf.Session() as sess:
        ...
        for i in range(10):
            ...
            while not done:
                ...
                if steps >= 1000: #same number of steps so all test trials are the same
                    break
            returns.append(totalr)
...
def train_behavior_cloning(expert_data,hidden_size):
    ...
    for i in range(num_iter):
        ...
        sess.run([train_step,mse_loss], feed_dict = {x:expert_data['observations'].astype(np.float)[0:batchsize,:], y_:expert_data['actions'].astype(np.float)[0:batchsize,0,:]})

        mse_results[i] = sess.run([mse_loss],{x:expert_data['observations'].astype(np.float)[0:batchsize,:], y_:expert_data['actions'].astype(np.float)[0:batchsize,0,:]})[0]
        if i%100==0:
